...
def test_export_cycles_csv(client: TestClient) -> None:
    ...
    _create_cycle(client, 15, "1000", channel_id)

    response = client.get("/export/cycles.csv")

    assert response.status_code == 200
    assert response.headers["content-disposition"] == 'attachment; filename="cycles.csv"'
...
def test_export_expenses_csv(client: TestClient) -> None:
    ...
    assert "Name,Amount,Cycle,Channel" in response.text
...
def test_export_transfers_csv(client: TestClient) -> None:
    ...
    cycle_id = _create_cycle(client, 15, "1000", from_id)
    ...
    assert "Cycle,From Channel,To Channel,Amount" in response.text
