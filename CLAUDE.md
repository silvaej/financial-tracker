...
- **`login.html` doesn't extend `base.html`** — `base.html` unconditionally renders the full authenticated app shell (rail nav, htmx-boosted), so the login page is a standalone template with its own `<html>`/`<head>` sharing only the compiled stylesheet.
- **`app/manage_users.py`** — CLI for account management (`create`, `set-password`, `assign-orphans`), run via `poetry run python -m app.manage_users <subcommand> <email>` against whichever `DATABASE_URL` is in the environment (same invocation style as `app/seed.py`). No web UI for any of this — deliberately, since there's no public registration. `assign-orphans` backfills any pre-auth rows (`user_id IS NULL`, from data created before this feature existed in a given environment) to a chosen user.
