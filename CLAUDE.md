...
- **User** — every other table has a nullable `user_id` FK to `users.id`; the app is multi-tenant, each user's channels/periods/expenses/etc. are fully isolated from every other user's. No public registration — accounts are created via the `app/manage_users.py` CLI, run manually by the operator.
